use crate::generate::{
    store_versioned_test_tfhe_10, TfhersVersion, INSECURE_SMALL_TEST_PARAMS_MS_NOISE_REDUCTION,
};
use crate::{
    HlClientKeyTest, HlServerKeyTest, TestDistribution, TestMetadata, TestParameterSet,
    HL_MODULE_NAME,
};
...
macro_rules! store_versioned_test {
    ($msg:expr, $dir:expr, $test_filename:expr $(,)? ) => {
        store_versioned_test_tfhe_10($msg, $dir, $test_filename)
    };
}
...
impl TfhersVersion for V1_0 {
    ...
    fn gen_hl_data() -> Vec<TestMetadata> {
        ...
        vec![TestMetadata::HlServerKey(
            HL_SERVERKEY_MS_NOISE_REDUCTION_TEST,
        )]
    }
}
